...
For the current single-operator and narrow-circle prioritization, read `docs/product/lean-roadmap.md`.
...
These items are no longer backlog candidates in strategic mode because a usable baseline already exists:
- SSL expiry monitoring inside HTTPS monitors
- richer assertions with `contains`, `regex`, `JSON path equals`, and `JSON path contains`
- first-pass agent fleet clarity in the operator UI
- first public status page baseline at `/status`

Strategic follow-up should now focus on growth directions that expand the product category rather than only polishing the current single-operator tool.

### 1. Expanded Check Types

Goal:
- make the product useful for a broader class of operational failures without turning it into a generic observability suite

Why:
- this is the most obvious gap versus strong self-hosted alternatives such as Uptime Kuma and Gatus
- it broadens the product faster than adding more workflow objects
- it strengthens the core monitoring value before more public or multi-user layers are added

Strategic scope:
- TCP checks
- DNS checks
- gRPC health or unary reachability checks
- heartbeat / cron-style push checks
- domain expiry monitoring
- latency / response-time threshold warnings

Out of scope for the first strategic phase:
- browser automation
- full synthetic monitoring
- deep protocol analyzers

### 2. Execution Policies And Multi-Region Monitoring

Goal:
- make remote agents a first-class strategic differentiator instead of just a transport detail

Why:
- this is where the product can differentiate from Uptime Kuma-class tools
- the existing split control plane and agent model already provide the foundation
- multi-location execution is a strong strategic feature without forcing enterprise process layers

Strategic scope:
- agent groups or regions
- per-monitor execution policy:
  - single designated agent
  - all assigned agents
  - majority / quorum semantics
  - fail-open vs fail-closed decision rules
- region-aware result presentation
- location-aware status evaluation

Out of scope for the first strategic phase:
- complex traffic steering
- automatic geo-placement
- advanced scheduler optimization

### 3. Service Model And Status Rollups

Goal:
- lift the product from individual monitors toward service-level understanding

Why:
- incidents, public communication, and grouped health become much clearer when there is a service layer
- this is the minimum useful abstraction above raw monitor lists
- it is more valuable strategically than only adding more per-monitor UX

Strategic scope:
- services or groups as first-class entities
- attach monitors to a service
- service health rollups from monitor states
- service summaries in operator UI
- public status rendering at the service level

Out of scope for the first strategic phase:
- CMDB behavior
- ownership trees
- dependency graphs with advanced graph logic

### 4. Incident Management And Communication

Goal:
- move from raw events into coherent outage lifecycle and communication

Why:
- this is the strategic layer that makes status pages, alerts, and service views feel like one product
- it improves both internal operations and external communication
- it is a natural second layer after service rollups exist

Strategic scope:
- incident objects and lifecycle
- incident updates
- service-linked incidents
- public incident communication on status pages
- maintenance and incident interplay

Out of scope for the first strategic phase:
- on-call scheduling
- postmortem tooling
- escalation policies comparable to PagerDuty

### 5. Public Status Pages V2

Goal:
- grow the public surface from one useful page into a configurable but still compact external communication layer

Why:
- the baseline page is already shipped
- the next strategic step is not “more polish” but stronger information architecture
- this becomes much more compelling once services and incidents exist

Strategic scope:
- multiple public pages
- page-level monitor or service selection
- public incidents and maintenance visibility
- configurable public wording and lightweight branding
- stable public slugs per page

Out of scope for the first strategic phase:
- per-customer portals
- white-label productization
- custom domain automation

### 6. Stable Integration Surface

Goal:
- make the product programmable and easier to embed in larger infrastructure workflows

Why:
- strategic growth requires something more stable than internal UI-only flows
- this increases product leverage without bloating the main operator UX
- it creates a cleaner path for import/export, automation, and third-party tooling

Strategic scope:
- stable public/admin API contracts
- outbound generic webhooks
- config export/import
- push-style integrations where appropriate

Out of scope for the first strategic phase:
- full Terraform ecosystem
...
### Epic X: Expanded Check Types

- [ ] T055 Add TCP check model, checker support, and UI
- [ ] T056 Add DNS check model, checker support, and UI
- [ ] T057 Add gRPC health or unary probe support
- [ ] T058 Add heartbeat / push monitor type for cron and job-style workflows
- [ ] T059 Add domain expiry monitoring
- [ ] T060 Add response-time threshold warnings and visibility
...
- [ ] T064 Add per-monitor execution policy model
- [ ] T065 Extend jobs payloads and scheduler logic for multi-agent execution
- [ ] T066 Define status semantics for quorum / majority / any-region failure
- [ ] T067 Show per-region execution results in the operator UI
- [ ] T068 Add tests for execution-policy evaluation and duplicate-result handling
- [ ] T069 Update architecture docs for multi-region execution semantics

### Epic G: Service Model And Status Rollups

- [ ] T070 Add service/group data model
- [ ] T071 Allow attaching monitors to a service
- [ ] T072 Implement service health rollup logic
- [ ] T073 Show service summaries and drill-down in the operator UI
...
- [ ] T077 Add incident objects linked to monitors and services
- [ ] T078 Implement incident updates and timeline entries
- [ ] T079 Add service-linked incident rendering in the operator UI
- [ ] T080 Expose public incident summaries on status pages
- [ ] T081 Define maintenance/incident interaction semantics
- [ ] T082 Add tests for incident lifecycle, updates, and public exposure
- [ ] T083 Update architecture, product, and runbook docs for incident communication

### Epic P: Public Status Pages V2

- [ ] T084 Add multiple public page model and stable slug support
- [ ] T085 Allow page-level monitor or service selection
- [ ] T086 Show incidents and maintenance on page-level public views
- [ ] T087 Add lightweight branding and wording controls
- [ ] T088 Add public-page management UI
- [ ] T089 Add auth-boundary and contract tests for multi-page public payloads
- [ ] T090 Update README and runbook for the expanded public surface

### Epic N: Stable Integration Surface

- [ ] T091 Define stable API surface boundaries and versioning expectations
- [ ] T092 Add outbound generic webhooks for monitor and incident events
- [ ] T093 Add config export/import for monitors, services, and public-page settings
...
### Sequence A
- Expanded Check Types

Reason:
- expands core monitoring value fastest
- closes the most obvious product-category gaps

### Sequence B
- Execution Policies And Multi-Region Monitoring

Reason:
- deepens the strongest structural differentiator already present in the product
- builds on the existing remote-agent foundation

### Sequence C
- Service Model And Status Rollups

Reason:
- makes the product easier to reason about at larger scale
- prepares public and incident layers to feel coherent

### Sequence D
- Incident Management And Communication

Reason:
- gives the product a stronger operational narrative once service and execution semantics are in place

### Sequence E
- Public Status Pages V2

Reason:
- public-facing growth benefits most once services and incidents exist
...
5. Expanded check types
6. Execution policies and multi-region monitoring
7. Service model and status rollups
8. Incident management and communication
9. Public status pages V2
10. Stable integration surface
11. Lightweight host/service context
