...
Use this file together with `docs/operations/production-topology.md`.
...
This feature is intended to catch certificate-renewal problems before they become uptime outages.

Current behavior:
- available on ordinary HTTPS monitors
- disabled by default
- configurable per monitor with a threshold in days
- warning/recovery notifications reuse the existing notification stack
- an SSL warning does not mark the monitor `DOWN` if the endpoint still passes its HTTP check

Operator flow:
1. Open an HTTPS monitor in the dashboard.
2. Enable `Monitor SSL certificate expiry`.
3. Set `Warn When <= Days`.
4. Save the monitor.
5. Wait for the next successful HTTPS check to populate certificate metadata.

What gets stored on each qualifying check:
...
- notification messages when the warning threshold is crossed or cleared

Practical recommendations:
- start with `14` days for public internet domains
- keep the feature off for plain HTTP targets
- only enable it on monitors where certificate drift actually matters operationally
- if a target sits behind a CDN or reverse proxy, the feature reports the external certificate seen by the checker, which is usually the desired behavior

Current implementation limits:
- HTTPS only
- no chain grading or OCSP/CRL diagnostics
- no separate public-status exposure yet
- warnings are deduplicated; they are not expected to spam on every check

Verification flow:
1. Enable SSL expiry monitoring on a known HTTPS monitor.
2. Wait for the next successful check.
3. Confirm that the monitor card shows an SSL summary.
4. Open monitor history and verify:
   - expiry date
   - days remaining
   - issuer
   - subject
5. If the certificate is already inside the threshold, confirm that a warning notification is emitted without changing the monitor from `UP` to `DOWN`.

Operational interpretation:
- no SSL summary on a newly enabled monitor usually means no successful HTTPS check has landed yet
...
- archive old backups
- document the host role change in `docs/operations/changelog-operations.md`
...
For the current production migration on `2026-03-12`, the recorded backup filenames are listed in `docs/operations/changelog-operations.md`.
